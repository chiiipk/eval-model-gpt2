...
# model
BASE_PATH="/teamspace/studios/this_studio/DTW-v2"
CKPT_TYPE="gpt2"
CKPT_NAME="gpt2-base"
...
TEACHER_MODEL_PATH="/teamspace/studios/this_studio/DTW-v2/model_hub/qwen/Qwen1.5-1.8B"
# data - modify this path to your self-correction dataset
DATA_DIR="/teamspace/studios/this_studio/self_correction_data"
# task
TASK="self_correction_dskd"
# hp
BATCH_SIZE=2
LR=0.00002
GRAD_ACC=4
...
DTW_GAMMA=2.0
# distiller
PROJECTOR_CONFIG_PATH="/teamspace/studios/this_studio/DTW-v2/configs/projector_config.json"
PROJECTOR_LR=0.001
# length
MAX_LENGTH=512
# runtime
PRECISION="bf16"
CRITERION="self_correction_dskd"
KD_OBJ="forward_kl"
CONFIG="${KD_OBJ}-${PRECISION}"
SETTING=criterion=${CRITERION}__${CONFIG}__teacher=${TEACHER_MODEL_NAME}__kd^rate=${KD_RATE}__kd^temp=${KD_TEMP}__kl^orig^rate=${KL_ORIG_RATE}__epoch=${EPOCH}__bsz=${BATCH_SIZE}x${GRAD_ACC}x${GPUS_PER_NODE}=$((BATCH_SIZE * GRAD_ACC * GPUS_PER_NODE * NNODES))__lr=${LR}__proj^lr=${PROJECTOR_LR}
...
OPTS+=" --wandb --wandb-project self-correction-kd --wandb-run-name ${CKPT_NAME}_${TASK}"
# model
...
OPTS+=" --kl-orig-rate ${KL_ORIG_RATE}"
OPTS+=" --kd-temperature ${KD_TEMP}"
OPTS+=" --original-model-path ${CKPT_PATH}"  # Use same model as reference
